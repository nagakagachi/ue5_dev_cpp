...
namespace ngl_procedual_mesh
{
	...
	public:
		...
		// RHI位置頂点バッファ
		naga::gpgpu::FComputeShaderPositionVertexBufferT PositionVertexBuffer;

		// RHIカラー頂点バッファ
		naga::gpgpu::FComputeShaderColorVertexBufferT ColorVertexBuffer;

		// RHIタンジェントノーマル頂点バッファ. デフォルトで4byte-PackedVector*2
		naga::gpgpu::FComputeShaderTangentsVertexBufferT<> TangentsVertexBuffer;

		// RHIUV頂点バッファ
		naga::gpgpu::FComputeShaderTexcoordVertexBufferT<> TexcoordVertexBuffer;

		// RHIインデックスバッファ
		naga::gpgpu::FComputeShaderIndexBuffer32 IndexBuffer;
...
IMPLEMENT_SHADER_TYPE(, FComputeShaderGrassUpdateCS, TEXT("/NagaExperiment/Shaders/compute_grass.usf"), TEXT("UpdateInstanceCS"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FComputeShaderGrassUpdateWithGridCellCS, TEXT("/NagaExperiment/Shaders/compute_grass.usf"), TEXT("UpdateInstanceWithGridCellCS"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FComputeShaderGrassGenerateMeshCS, TEXT("/NagaExperiment/Shaders/compute_grass.usf"), TEXT("GenerateMeshCS"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FComputeShaderGrassGenerateCountCS, TEXT("/NagaExperiment/Shaders/compute_grass.usf"), TEXT("GenerateEntityCountInfoCS"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FComputeShaderGrassAppendDirectCS, TEXT("/NagaExperiment/Shaders/compute_grass.usf"), TEXT("AppendDirectInstanceCS"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FComputeShaderGrassGatherToGridCellCS, TEXT("/NagaExperiment/Shaders/compute_grass.usf"), TEXT("GatherToGridCellCS"), SF_Compute)
...
// シェーダコードとGlobalShaderクラスを関連付け
IMPLEMENT_SHADER_TYPE(, FClearSortedBufferInfoCS, TEXT("/NagaExperiment/Shaders/compute_sorted_buffer_util.usf"), TEXT("ClearSortedBufferInfo"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FClearUint2BufferCS, TEXT("/NagaExperiment/Shaders/compute_sorted_buffer_util.usf"), TEXT("ClearUint2Buffer"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FGenSortedBufferInfoUint2CS, TEXT("/NagaExperiment/Shaders/compute_sorted_buffer_util.usf"), TEXT("GenSortedBufferInfo"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FGenSortedBufferKeyDiffUint2CS, TEXT("/NagaExperiment/Shaders/compute_sorted_buffer_util.usf"), TEXT("GenSortedBufferKeyDiff"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FExclusiveScanPerGroupUintCS, TEXT("/NagaExperiment/Shaders/compute_sorted_buffer_util.usf"), TEXT("ExclusiveScanPerGroup"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FCompleteBlockExclusiveScanUintCS, TEXT("/NagaExperiment/Shaders/compute_sorted_buffer_util.usf"), TEXT("CompleteBlockExclusiveScan"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FGenEnableHashInfoBufferCS, TEXT("/NagaExperiment/Shaders/compute_sorted_buffer_util.usf"), TEXT("GenEnableHashInfoBuffer"), SF_Compute)
...
// シェーダコードとGlobalShaderクラスを関連付け
IMPLEMENT_SHADER_TYPE(, FComputeSetRandomValueUint2Cs, TEXT("/NagaExperiment/Shaders/compute_sort_dispatch_reduce.usf"), TEXT("SetRandomValue_Uint2"), SF_Compute)
...
public:
		...
		, buffers_(ERHIFeatureLevel::SM5)
	{
		...
		// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		{
			...
			auto msb = naga::math::Msb( FMath::Max( uint32(component_->grass_max_count_ - 1), 1u ) );
		...
		TArray<naga::gpgpu::UintVec2> entity_hash_array;
...
protected:
	...
	naga::gpgpu::FStructuredBufferResource<GrassEntityInfo>	entity_base_info_buffer_;

	// ソート済みバッファの未使用要素の個数などを格納するバッファ.
	naga::gpgpu::FStructuredBufferResource<GrassEntityCountInfo>	sorted_entity_counter_buffer_;

	// インスタンス追加用情報バッファ
	naga::gpgpu::FStructuredBufferResource<naga::gpgpu::GrassEntityAppendInfo>	entity_append_buffer_;


	// ここからは汎用のソートやソート済みバッファの情報取得用
	// 転置シェーダのために入出力二つのバッファが必要.
	naga::gpgpu::FStructuredBufferResource<naga::gpgpu::UintVec2>			sorted_hash_buffer_;

	// タイルベース転置BitonicSortオブジェクト
	naga::gpgpu::GpuSimpleOutOfPlaceTiledTransposeBitonicSortUint2	simple_tiled_transposed_bitonic_sorter_;
	// Dispatch数削減BitonicSortオブジェクト
	naga::gpgpu::GpuDispatchReduceBitonicSortUint2					dispatch_reduce_bitonic_sorter_;


	// Exclusive Scan生成用の作業バッファ
	// ソート済みハッシュの変化位置に1それ以外0なバッファを生成したりグループ単位Scanバッファの各グループ担当分の最後の要素を書き込んで全体Scanの入力とするバッファに使う.
	naga::gpgpu::FStructuredBufferResource<uint32>			sorted_hash_exclusive_scan_work;
	naga::gpgpu::FStructuredBufferResource<uint32>			sorted_hash_exclusive_scan;
	// ブロック単位Scan時にブロック毎の合計値を書き込むバッファ. 作業用に二つ
	naga::gpgpu::FStructuredBufferResource<uint32>			sorted_hash_sums_exclusive_scan0_;
	naga::gpgpu::FStructuredBufferResource<uint32>			sorted_hash_sums_exclusive_scan1_;

	// 有効なハッシュ値に関する情報のバッファ
	// 0番要素に有効なハッシュ値の個数N
	// 1番以降にN個の有効なハッシュの値とそのハッシュ値が開始するsorted_hash_buffer_上のインデックスが格納される
	// 0番に有効要素数を格納しているのは二分探索時に必要な情報を集約するため.
	naga::gpgpu::FStructuredBufferResource<naga::gpgpu::UintVec2>			enable_hash_buffer_;

	// グリッドセルの情報
	naga::gpgpu::FStructuredBufferResource<GridCellInfo>		grid_cell_data_buffer_;

	// ソート済みバッファHashの各ビットMSB開始インデックスバッファ. 32要素. 主に末尾の無効要素へのアクセスのため.
	naga::gpgpu::FStructuredBufferResource<uint32>			sorted_hash_bit_msb_index_buffer_;
...
void UComputeShaderMeshComponent::AddCurrentFrameEmissionBuffer(const FVector& pos, const FVector& dir, float length, float life_sec_init)
{
	...
	naga::gpgpu::GrassEntityAppendInfo newObj;
