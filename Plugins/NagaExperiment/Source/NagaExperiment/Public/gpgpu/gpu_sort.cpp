...
namespace naga::gpgpu
{
	...
	IMPLEMENT_SHADER_TYPE(, FDispatchReduceBitonicSortUint2, TEXT("/NagaExperiment/Shaders/compute_sort_dispatch_reduce.usf"), TEXT("BitonicSortDispatchReduceV0_Uint2"), SF_Compute)
	...
	void GpuDispatchReduceBitonicSortUint2::Run(FRHICommandListImmediate& cmdlist, ERHIFeatureLevel::Type FeatureLevel, FStructuredBufferResource<UintVec2>& buffer)
	{
		...
		// Dispatch
		{
			...
			SCOPED_DRAW_EVENT(cmdlist, TEXT("BitonicSortDispatchReduce"));
			for (uint32_t dispatch_index = 0; dispatch_index < precomp_param_.dispatch_count_; ++dispatch_index)
			{
				...
				SCOPED_DRAW_EVENT(cmdlist, *GpuEventName);
	...
	*/
	...
	// シェーダコードとGlobalShaderクラスを関連付け
	IMPLEMENT_SHADER_TYPE(, FSimpleBitonicSortUint2, TEXT("/NagaExperiment/Shaders/compute_sort.usf"), TEXT("BitonicSort_Uint2"), SF_Compute)
	...
	*/
	...
	// シェーダコードとGlobalShaderクラスを関連付け
	IMPLEMENT_SHADER_TYPE(, FOutOfPlaceTiledTransposeUint2, TEXT("/NagaExperiment/Shaders/compute_sort.usf"), TEXT("Transpose_Uint2"), SF_Compute)
	...
	void GpuSimpleOutOfPlaceTiledTransposeBitonicSortUint2::Run(FRHICommandListImmediate& cmdlist, ERHIFeatureLevel::Type FeatureLevel, FStructuredBufferResource<UintVec2>& buffer)
	{
		...
		{
			SCOPED_DRAW_EVENT(cmdlist, *FString::Format(TEXT("BitonicSort"), { 0 }));
			...
			{
				SCOPED_DRAW_EVENT(cmdlist, *FString::Format(TEXT("BitonicSort_{0}"), { i }));
